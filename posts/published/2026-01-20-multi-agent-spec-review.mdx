...
I recently had a good time with using sub-agents to prevent context-rot and introduce diverse perspectives when coding with agents.
...
Essentially, spec-driven development is Claude Code plan mode taken up a notch. Today (January 20, 2026), Claude will actually write a spec of sorts whenever you plan. Typically, spec-driven development with Claude gives the opportunity to be a lot more detailed and consistent across all of your plans and specs, which is great for reaching a deterministic workflow.

> [!NOTE] Why are people obsessed with being deterministic with AI?
>  Well AI is very exciting, but every time you open a context window, it's like speaking to a new person that works at the same company; the ideas/outcomes you'll arrive at will vary widely depending on how that person (agent) happens to interpret things on that day.
>
...
That's when I realized, what if I created multiple sub-agents with separate personas who only received the written spec with no steer from the conversation I've had with Claude prior to writing the spec, so it could review it purely on the grounds of the written spec and its own persona...
...
2. 6 `spec-reviewer` agents, each with a different persona, values and interests
...
## Escalation path on conflict or severe risk
...
1. Are not biased by the prior conversation I've had with Claude (especially on how I think my idea is so great 😅)
2. Can be contextulized with a different set of values and priorities that I want to see represented in the spec review
3. Can use a different model to the author for even more diversity of thought and optimal use of token budget
